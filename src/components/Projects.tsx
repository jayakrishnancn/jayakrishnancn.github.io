...
const Projects = () => {
  return (
    <>
      <h1 className="text-center uppercase mt-28 mb-10  underline">Projects</h1>
      <div
        id="projects"
        className="grid sm:grid-cols-2 md:grid-cols-3 gap-3 text-justify"
      >
        {PROJECTS.sort((a, b) => b.year - a.year).map(project => {
          ...
          return (
            <div
              key={`projects-${title}`}
              className="flex flex-col mt-3 justify-between group p-5 border text-muted hover:border-red-400 bg-gray-50 rounded-md transform shadow-sm hover:shadow-md transition-all hover:-translate-y-2 cursor-default"
            >
